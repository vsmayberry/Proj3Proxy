    ...
    for (;;) {
        ...
        else if(!connected && fd2==-1 && dead)
        {
            ...
            while(dead)
            {
                ...
                memset(&a, 0, sizeof(a));
                a.sin_port = htons(forward_port);
                a.sin_family = AF_INET;

                if (!inet_aton(argv[1], (struct in_addr *) &a.sin_addr.s_addr)) {
                    perror("bad IP address format");
                    close(s);
                    return -1;
                }
                ...
                else
                {
                    ...
                    printf("SEQ = %d, QUE %d, ACK %d\n", data -> seq_num, sequence_number, ack_number);
